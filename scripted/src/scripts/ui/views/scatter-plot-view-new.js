...
Exhibit.ScatterPlotView.prototype._reconstruct = function() {
    ...
    currentSize = collection.countRestrictedItems(); //gives you the items after all of the filtering has gone through
	...
	//if selected items is greater than 0, then construct a scatter plot
    if (currentSize > 0) {
        ...
        prepareData();
        this._clickHandler(flotr, scatterDiv, xyToData); //binding the clickHandler/listener to the view
        this._createFlotrScatter(flotr,scatterDiv, dataToPlot, xyToData, legendLabels); //call flotr scatterplot constructor and pass in data, also the dictionary which opens the popup, etc etc
...
Exhibit.ScatterPlotView.prototype._clickHandler = function(flotr, container, xyToData) {
	...
	//checks and finding data item that has been clicked
    $("body").click( function(e) {
        ...
        if (!popupPresent) {
            ...
            if (distance < 10) {
                ...
                popupPresent = true;
                Exhibit.ViewUtilities.openBubbleWithCoords(e.pageX, e.pageY, items, self.getUIContext()); //creates bubble (using openbubble function that baiaboo added, but will be changing)
            }
        }
    });
}
...
Exhibit.ScatterPlotView.prototype._createFlotrScatter = function(flotr, container, dataToPlot, xyToData, legendLabels) {
    ...
    for (key in dataToPlot) {
        dataList.push({
            data : dataToPlot[key], label : legendLabels[i], points : { show : true }, lines : { show : false },color : key 
        });
        i++
    }
