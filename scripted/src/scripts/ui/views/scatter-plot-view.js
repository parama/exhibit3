...
Exhibit.ScatterPlotView = function(containerElmt, uiContext) {
	...
	//making an instance of the view
	var view = this;
	...
	//defining global variables for your view. 
	this._accessors = {
	...
	//this tells us what to do when items change - ie what happens when the collection changes. (after filtering/selecting some group of items)
	//event handler
    this._onItemsChanged = function(){
    	view._reconstruct(); //call view reconstruct each time selected items group changes
    };
    
	//we are listening to the event (that the collection is changing)
    $(uiContext.getCollection().getElement()).bind(
    	"onItemsChanged.exhibit", //when selected items change
    	view._onItemsChanged //do this - call this,_onItemsChanged reconstruct the view
    );
    
    this.register(); //you have to register your view to the whole exhibit view panel
};

//set parameters of the view/plot type data points (data vals, types, etc)
Exhibit.ScatterPlotView._settingSpecs = {
    "plotHeight":   { type: "int",   defaultValue: 400 },
	"plotWidth":   { type: "int",   defaultValue: 400 },
...
//more user specified parameters, but this involves examining their input data
    ...
    {   "accessorName": "getXY",
        "alternatives": [
            {   "bindings": [
                    {   "attributeName":  "xy",
                        "types":          [ "float", "float" ],
                        "bindingNames":   [ "x", "y" ] //so user might say, ex:x="birthrate" in the factbook example
                    }
                ]
            },
            {   "bindings": [
...
//both of these are required, basically constructor functions to create an specific scatter plot

//can be used for batch graphs if you are using a script
...
//created from the DOM by the user in the html file
Exhibit.ScatterPlotView.createFromDOM = function(configElmt, containerElmt, uiContext) {
	...
	//these things make user set parameter data available to you
...
Exhibit.ScatterPlotView._configure = function(view, configuration) {
	//not sure why these are here, can check whether deleting these matters...
	...
	//initializing the functions/data initializing the x and y functs...these help us scale values, not sure whether it should or needs to be done here...
...
//specific functions that are used in scatter plot and bar chart
...
//does checks so that code won't crash
...
Exhibit.ScatterPlotView.prototype._reconstruct = function() {
    ...
    if (currentSize > 0) {
         ...
         * dataToBePlotted = list of the data points
         */
       ...
       $(container).bind("click",function(e){
        ...
        Exhibit.WindowManager.registerEvent(container, "click", function(elmt, e, target) {
